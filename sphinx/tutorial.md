...
the `VaspObaSet` library, written by Kumagai largely on the basis of [pymatgen](http://pymatgen.org) developed by Materials Project team, is used.
...
```
PMG_MAPI_KEY is required for querying POSCARs of competing materials as shown later.
...
The wavefunctions of the PHS widespread to several million atoms, so we need such supergiant supercells to estimate their thermodynamical transition levels.  
...
Dots on the left and right sides are eigenenergies in the defect (thus V<sub>Ba</sub><sup>+1</sup> here) and the perfect supercells, respectively.
