...
**Note1: Units used in PyDefect are eV for energy and Angstrom for length following the vasp convention.**
...
Note, if competing phases are gases, we need to change `ISIF` to 2 so as not to relax the lattice constants (see [vasp manual](https://cms.mpi.univie.ac.at/wiki/index.php/ISIF)), 
and `KPOINTS` requires only the Gamma point sampling.
...
1. The antisites and substituted defects are determined from the difference of the electronegativity. 
   Although default is 1.0, you can change it using `-e` option, e.g., 1.5.
   ```
...
If you want to calculate only oxygen vacancies, you can restrict the calculated defects with a python regular expression,
```
...
In each directory, we can find the `defect_entry.json` file, which contains the information about the point defect obtained before the first-principles calculations.
...
In some cases, complex defects or some peculiar defects, e.g., DX centers, need to be generated.
Then, we need to construct `POSCAR`s by ourselves.
However, PyDefect can generate `defect_entry.json` by parsing the defect POSCAR and INCAR files.

First, make POSCAR as you want and type the following command to generate the other vasp input set.
```
python ~/my_bin/pydefect/pydefect/main.py vos -t defect -x pbesol --dirs . --charge -1
```
In this case, the charge is assumed to be -1.
And then, type 
```
...
We also suggest the users to user Gamma version of vasp if the k-point sampling is only Gamma point.
...
Here, we note that the name of `perfect/` has special meaning, so users must use this name for the supercells w/o defects.
...
After the convergence, we can see band edge information at the spin up (UP) and down (DOWN) channels, which will be explained later.

### 10. Corrections of defect formation energies in finite-size supercell 
When supercell method is adopted, because of interaction between a defect, its images, and background charge, the obtained total energies for **charged defects** are not properly estimated
Therefore, we need to correct the total energies of charged defect supercells to predict those in dilute limit.
...
Bear also in mind that this command takes some time, so we recommend the users to prepare coffee or go on a walk outside.

The correction method used in PyDefect at this moment is the so-called extended Freysoldt-Neugebauer-VandeWalle (eFNV) method.
