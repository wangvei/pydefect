...
class DefectEnergies:
    ...
    """
    def __init__(self, defect_energies, transition_levels, vbm, cbm,
                 supercell_vbm, supercell_cbm, title=None):
        """
        Args:
            defect_energies (defaultdict):
    ...
    def from_files(self, unitcell, perfect, defects, chem_pot, chem_pot_label,
                 system=""):
        """
        ...
        self._supercell_cbm, self._supercell_vbm = \
            perfect.eigenvalue_properties[1:3]

        self._title = system + " condition " + chem_pot_label

        # chemical potentials
        relative_chem_pots, standard_e = chem_pot
        relative_chem_pot = relative_chem_pots[chem_pot_label]

        # Calculate the defect formation energies at the vbm
...
# TODO: add site_symmetry before and after structure optimization.
DefectSite = namedtuple("DefectSite", ("num_site", "spin"))
...
class Constraint(Enum):
    """
    Supported concentration calc types in pydefect
    """
    none = "none"
...
class DefectConcentrations:
    """
    A class related to a set of defect concentrations
    """
    def __init__(self, temperature, e_f, p, n, defect_concentration):
    ...
    def from_defect_energies(self, defect_energies, temperature, unitcell,
                             defect_sites, constraint, e_f=None,
                             defect_concentration=None, title=None):
        """
        Calculates defect formation energies.
        Args:
            ...
            temperature (float):
            unitcell (UnitcellDftResults):
            defect_sites (DefectSites):
            constraint (ConcentrationConstraint):
            e_f (float): Fermi level in the absolute scale.
            defect_concentration (defaultdict):
            title (str): A system name used for the title
        """
        self._defect_energies = defect_energies
        ...
        if constraint == Constraint.none:
            if e_f is None:
                raise ValueError("Fermi level is needed for defect "
                                 "concentration calculations.")
            self._defect_concentration = \
                self.defect_concentration(temprature, e_f, num_sites)
    ...
    def defect_concentration(self, temperature, e_f, num_sites_filename):
        ...
        for name in self._defect_energies.keys():
            ...
            if self._original_concentration is None:
                ...
                dc = self._original_concentration.defect_concentration[name]
                total = sum(dc.values())
                ratio = {}
                for charge in self._defect_energies[name].keys():
                    energy = self._defect_energies[name][charge] + e_f * charge
    ...
    def equilibrium_concentration(self, temperature, num_sites_filename,
        ...
        for iteration in range(max_iteration):
            ...
            if np.abs(charge_sum / max_concentration) < threshold:
                ...
                if self._original_concentration is None:
                    self._original_concentration = c
                else:
                    self._quenched_concentration = c
                return True
    ...
    def __str__(self):
        if self._quenched_concent:
            cs = [self._original_concentration, self._quenched_concent]
        else:
            cs = [self._original_concentration]
...
class DefectPlotter:
    ...
    def __init__(self, defects,
        ...
        # Note: vbm, cbm, perfect_vbm, perfect_cbm are in absolute energy.
        self._vbm, self._cbm = defect_energy.
        # The band edges in the perfect supercell calculation
    ...
    #     Va_O1_0, Va_O1_1 and Va_O1_2 are fixed.
        ...
        #             "{} is not in {}.".format(name, num_sites_filename))
            ...
            #             "set. Unset the concentrations first.")
                ...
                # dc = self._original_concent.defect_concentration[name]
                # total = sum(dc.values())
                # ratio = {}
                # for charge in self._defect_energies[name].keys():
                #     energy = self._defect_energies[name][charge] + e_f * charge
