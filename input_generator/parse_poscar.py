...
def make_point_defects(poscar_name, removed_add, added_coord, added_atom_symbol, 
                       output_poscar_name, r_param=False, header="", empty=True):
    # removed_add : atom index
    # added_coord : fractional coordinates:
    # added_atom_symbol : element name
    # rand[0]: Cutoff [A] to determine atoms for considering the randomization.
    # rand[1]: Maximum distance for the random displacement.
    # -------------------------------------------------------------------------
    ...
    if removed_add:
        for i in range(len(num_atoms)):
            if sum(num_atoms[0:i + 1]) >= removed_add: 
                num_atoms[i] -= 1
                break

        # Removed or replaced site. This is also used for empty sphere.
        vacant_coord = atom_pos[removed_add - 1]
        atom_pos = np.delete(atom_pos, removed_add - 1, axis=0)

    # ------ define defect site ------
    # defect_site: vacancy + intersitial
    if removed_add and added_coord:
    ...
    if r_param is not False:
        atom_pos, displaced_atom_index = random_displaces(lattice_vectors, 
                                 atom_pos, defect_coord, r_param[0], r_param[1])
    else:
        displaced_atom_index = ["" for i in range(len(atom_pos))]  

    # "interstitial" or "substitutional" 
    if (not removed_add and     added_coord and added_atom_symbol) \
    or (    removed_add and not added_coord and added_atom_symbol):
...
#    if (removed_add and not added_coord and not added_atom_symbol and empty) \
#    or (removed_add and     added_coord and     added_atom_symbol and empty):
