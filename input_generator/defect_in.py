...
class DefectSetting():
    ...
    def __init__(self, structure, irreducible_sites, dopant_configs, 
                 ...
                 displace, cutoff, symprec, oxidation_states, 
                 electronegativity):
...
#       This must be as_dict(); othewize irreducible_sites show pointers. 
...
#        return cls({k: v for k, v in d.items()})
        return cls(d["structure"], irreducible_sites, d["dopant_configs"],
                   ...
                   d["included"], d["excluded"], d["displace"], 
...
#        print(loadfn(filename))
    ...
    def from_defect_in(cls, poscar="DPOSCAR", defect_in_file="defect.in"):
        ...
        with open(defect_in_file) as defect_in:
            for l in defect_in:
                ...
                if line == []: 
                    ...
                    irreducible_name = line[3]
                    # remove irreducible index number from irreducible_name
                    element = ''.join(
                                     [i for i in irreducible_name if not i.isdigit()])
                    # Skip one line which shows the representative index.
                    # First_index atom is assumed to represent irreducible_name atoms.
                    defect_in.readline()
                    ...
                    irreducible_sites.append(IrreducibleSite(irreducible_name, element, 
                    ...
                    b = [float(''.join(i for i in line[i] if i.isdigit() or i == '.')) for i in range(2, len(line))]
        ...
        return cls(structure, irreducible_sites, dopant_configs, antisite_configs, 
                   interstitial_coords, included, excluded, displace, 
...
class DefectInMaker():
    ...
    def __init__(self, structure, dopants, interstitial_coords, is_antisite, 
                 EN_diff, included="", excluded="", displace=0.2, cutoff=3.0, symprec=0.01):
...
#                                        Element(s).common_oxidation_states[-1]
    ...
    def from_structure_file(cls, poscar, dopants=[], interstitial_coords=False,
         ...
         excluded=None, displace=0.2, cutoff=3.0, symprec=0.01):
        """
        ...
        return cls(structure, dopants, interstitial_coords, is_antisite, 
                   EN_diff, included, excluded, displace, cutoff, 
                   symprec)
