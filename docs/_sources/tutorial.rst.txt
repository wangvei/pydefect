...
we use :code:`vise` (= :code:`vasp integrated supporting environment`) code written by Kumagai largely on the basis of [pymatgen](http://pymatgen.org), which can generate them for various tasks and exchange-correlation (XC) functionals.
...
Here, :code:`PMG_MAPI_KEY` is required for querying :code:`POSCAR`s of competing materials as shown later.

Input files for optimizing a unitcell using the PBEsol functional are generated by the following command.
Here, the pbesol functional is a default, so if one wants to change the XC functional, use the :code:`-x` option.
...
For example, for :code:`vasp_set` and :code:`defect_vasp_set` which appear later, we can use :code:`potcar_set` key as follows
If you write :code:`vise.yaml` as
...
There are so many keys for :code:`pydefect.yaml`, which is written at the top of the :code:`main.py` file.
...
`vise` also provides the plotters of BS and DOS based on with :code:`plot_band` (= :code:`pb`) and :code:`plot_dos` (= :code:`pd`) sub-commands.
...
using the :code:`unitcell_results` (= :code:`ur`) sub-command.

::

    pydefect ur --static_diele_dir dielectric --ionic_diele_dir dielectric --band_edge_dir band --volume_dir dielectric --total_dos_dir dos --json_file unitcell.json -o OUTCAR-finish -p POSCAR-finish -v vasprun.xml

In this command, we suppose that OUTCAR and CONTCAR are renamed to OUTCAR-finish and POSCAR-finish, respectively, as an example, using :code:`-o` and :code:`-p` options.
If you write the following in the :code:`pydefect.yaml` file,
...
Using the :code:`chempotdiag` library developed and managed by Akira Takahashi,
...
::
    ...
    pydefect vos -t structure_opt -is ENCUT 520 --dirs *_mp*/
...
and :code:`KPOINTS` to the &Gamma; point sampling.</p>

In such case, type as follows,

::

    pydefect vs -t structure_opt -is ENCUT 520 ISIF 2 -vs_kw is_cluster True --dirs O2molecule_pydefect
...
::
    ...
    vise cpd -v */ -c MgO -y -s cpd.pdf

If you rename the CONTCAR and OUTCAR files to e.g., POSCAR-finish and OUTCAR-finish, type

::

    vise cpd -v */ -p POSCAR-finish -o OUTCAR-finish -c MgO -y -s cpd.pdf

See help of :code:`chempotdiag` for more other options.
...
We then create a supercell and an initial setting file for point defects with the :code:`initial_setting` (= :code:`is`) sub-command.
pydefect recommends a nearly isotropic (and sometimes cubic-like) supercell composed of moderate number of atoms.
For this purpose, use the :code:`recommend_supercell` (= :code:`rs`) sub-command,

::

    pydefect is

Then, :code:`DPOSCAR` that is the POSCAR for the supercell, respectively, is generated.
:code:`UPOSCAR` is also generated if the input structure is different from the standardized primitive cell.
Although pydefect constructs :code:`SPOSCAR` based on the *conventional* unitcell by default,
it can also expand the *primitive* unitcell with :code:`-pr` option.
...
Another exception is the tetragonal cell, where 45 degree rotated supercells are allowed, which is also implemented in :code:`pydefect`.

In :code:`pydefect`, when a user wants to use specific supercell, she/he needs to specify the cell matrix, e.g.,

::

    pydefect is --matrix 2 1 1


With :code:`is` sub-command, we can build the :code:`defect.in` file, which contains the full information on what kind of defects are to be generated automatically.
At the same time, :code:`DPOSCAR`, POSCAR used for the defect calculations, is created.
...
The :code:`Coordination` is shown for the atoms whose distances from the defect are less than 1.3 times of the minimum coordination distance
for each atom in the perfect cell.
This parameter is also controlled with the :code:`pydefect.yaml` file.

If we want to add dopants *a posteriori*, we can type as follows.

::

    pydefect is --print_dopant Na
...
By inserting this with an editor to :code:`defect.in` and modify :code:`Substituted defects` as follows
...
There are so many tips related to :code:`defect.in`.
1. The antisites and substituted defects are determined from the difference of the electronegativity.
   Although default is 1.0 <span>&#8491;</span>, you can change it using :code:`-e` option to e.g., 1.5.
...
3. By default, positions of atoms neighboring a defect are perturbed such that the symmetry is lowered, but it is unwanted in some cases.
   Then, :code:`Maximum Displacement` is simply set to 0.
...
:code:`pydefect` implements a recommendation of the interstitial sites using the unitcell charge density
using the :code:`ChargeDensityAnalyzer` class implemented in :code:`pymatgen`.
To use this, we need to generate :code:`CHGCAR` based on the :code:`POSCAR` file or :code:`UPOSCAR` file if generated.
(Of course, if you already have :code:`CHGCAR` at the unitcell calculations, you can use it, but be sure the structure must be the same as that used here.)
For this purpose, make `chgcar/` and copy UPOSCAR to :code:`chgcar/POSCAR` and type

::

    vise vs -t structure_opt -uis LCHARG True LWAVE False -vise_opts standardize_structure False

Then, type

::

    pydefect i --chgcar CHGCAR
...
To add the interstitial site at e.g., 0.25  0.25  0.25, we use the :code:`interstitial` (= :code:`i`) sub-command like

::

    pydefect i -c 0.25 0.25 0.25


:code:`interstitials.yaml` is then generated, which show information related to the interstitial sites.
...
::
    ...
    pydefect is --interstitial_sites i1 --dopants Na
...
In :code:`pydefect`, we can define complex defects.
Let's consider the Mg-O pair in MgO.
In the former process, the :code:`DPOSCAR` composed of 64 atoms is already generated.
Now, we consider Mg at (0, 0, 0) (index: 0) and O at (0.25, 0, 0) (index: 63).
By typing

::

    pydefect cd -r 0 63 --name divacancy --extreme_charge_state 0

we obtain the :code:`complex_defects.yaml`, which shows,
...
Then, modify :code:`defect.in` file as follows.
...
============================================================================
We next create directories for point-defect calculations by the :code:`defect_vasp_set` (= :code:`dvs`) sub-command like
...
With this command, a lot of directories are created, including :code:`perfect`.
...
::
    ...
    pydefect dvos -kw "Va_O[0-9]?_[0-9]+"
